...
# install etcd
cd ${SCRIPT_PATH}
if [ -f "$DEPLOY_PATH/conf/client-key.pem" ]; then
    log WARN "certification already exist"
    rsync -cir --delete --bwlimit=$BWLIMIT "$CONTENT_PATH/" "$DEPLOY_PATH/" --exclude=/logs --exclude=/data --exclude=/conf/*.pem --exclude=/tmp  --exclude="/.*";
else
    rsync -cir --delete --bwlimit=$BWLIMIT "$CONTENT_PATH/" "$DEPLOY_PATH/" --exclude=/logs --exclude=/data  --exclude=/tmp  --exclude="/.*";
fi
