...
#==============================================================================
#description     :node join to cluster v1.16.x.
...
# Master
if [ -z "$master" ]; then
  echo "master in install.properties is not set";
  exit 1;
fi

if [ 0 =  "$(grep MASTER_VIP ${install_config} | wc -l)" ]; then echo "master ip $master";
else
  echo "[@IDC_dockin_MASTER_VIP] in install.properties is not be replaced";
  exit 1;
fi

# token
if [ -z "$token" ]; then
  echo "token in install.properties is not set";
  exit 1;
fi
...
if [ -z "$env" ];
then
    kubeadm join $master:6443 --token=$token --discovery-token-unsafe-skip-ca-verification --v=4;
else
    if [ $env == "sit" ];
    then
