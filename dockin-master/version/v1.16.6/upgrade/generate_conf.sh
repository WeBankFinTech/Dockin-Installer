...
#==============================================================================
#description     :install kubernetes master v1.13.12.
#author		       :
#linux           :centos7
#user            :root
...
if [ -z "$etcd_ip_list" ]; then
  log ERROR "etcd_ip_list in install.properties is not set";
  exit 1;
fi
...
IFS=',' read -ra etcd_arr <<< "$etcd_ip_list"
for i in "${etcd_arr[@]}"; do
  etcd_ip_list_dest="${etcd_ip_list_dest}    - http\:\/\/${i}\:2379\n"
done

etcd_ip_list_dest=${etcd_ip_list_dest::-2}
sed -i "s/\[dockin_ETCD_IP_LIST\]/$etcd_ip_list_dest/g" tmp/master-config/kubeadm-config.yaml
