...
echo "backup images"
docker save $(docker images --format "{{.Repository}}:{{.Tag}}" | grep -v webank) -o imagebackup.tar
...
echo "uninstall docker 17.03/18.09..."
...
echo "restore images"
docker load --input imagebackup.tar
